...
var TableHandler = declare(_Plugin, {
	...
	connectDraggable: function(){
		...
		//
		if(!dojo.isIE){
			// 0 && console.warn("Drag and Drop is currently only detectable in IE.");
			return;
		}
	...
	_tempStoreTableData: function(type){
		...
		if(type===true){
			...
			this.tableData = null;
			 ...
			 0 && console.warn("_tempStoreTableData must be passed an argument");
		}else{
			// type is a number/ms
	...
	_tempAvailability: function(type){
		...
		if(type===true){
			...
			this.availableCurrentlySet = false;
			 ...
			 0 && console.warn("_tempAvailability must be passed an argument");
		}else{
			// type is a number/ms
			this.availableCurrentlySet = true;
			setTimeout(dojo.hitch(this, function(){
